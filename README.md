...
* [nginx-strip-filter-module](https://github.com/myguard-labs/nginx-strip-filter-module) - Dynamic response-body minifier — strips newlines and redundant whitespace from HTML/CSS/JS responses.
* [nginx-zstd-module](https://github.com/myguard-labs/nginx-zstd-module) - Zstandard (zstd) compression and decompression filters for nginx responses.
* [nginx-http-sentinel-module](https://github.com/myguard-labs/nginx-http-sentinel-module) - (experimental) PREACCESS module that scores every client from weighted signals and acts on the verdict — tarpit bots, block scanners, PoW-challenge grey-area requests.
...
* [nginx-lua-waf-kit](https://github.com/fabiocicerchia/nginx-lua-waf-kit) - Opt-in Lua WAF modules for nginx-lua: rate limiting, bot scoring, edge JWT, geo/ASN policy, mirroring.
...
* [nginx-config-auditor](https://git.canutethegreat.com/CanuteTheGreat/nginx-config-auditor) - Free, stdlib-only Python 3 static analyzer for nginx configs: flags directory-listing exposure, weak TLS protocols/ciphers, missing security headers (HSTS, X-Content-Type-Options, CSP, etc.), version disclosure, and other common misconfigurations. No network calls, no dependencies, read-only. Point it at a config file, a directory, or a running Docker container. MIT licensed.
